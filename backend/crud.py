...
class RestDwelling(Dwelling, db.Model):
    ...
    deals = db.relationship("RestDeal", backref="rest_dwelling")
...
def get_filtered_dwelling():
    ...
    result = filter_dwellings(price, year, dwellings)
    return jsonify({'dwellings': result})
...
# @cross_origin()
def add_dwelling():
    new_dwelling = RestDwelling(request.json['img'], request.json['title'],
...
def update_dwelling(id):
    ...
    dwelling.title = request.json['brand']
    dwelling.title = request.json['year']
...
class RestDeal(Deal, db.Model):
    id = db.Column(db.Integer, primary_key=True)
    deal_date = db.Column(db.Date, unique=False)
    ...
    dwelling_id = db.Column(db.Integer, db.ForeignKey('rest_dwelling.id'))  # important

    active = db.Column(db.Boolean, unique=False)
    fines = db.relationship("RestFine", backref="rest_deal")

    def __init__(self, deal_date, price, rent_time, collateral_amount, user_id, dwelling_id, active):
        super().__init__(deal_date, price, rent_time, collateral_amount, user_id, dwelling_id, active)


class DealShema(ma.Schema):
    class Meta:
        fields = ('active', 'collateral_amount', 'deal_date', 'dwelling_id', 'id', 'price', 'rent_time', 'user_id')
...
# @cross_origin()
def add_deal():
    new_deal = RestDeal(request.json['deal_date'], request.json['price'], request.json['rent_time'],
                        request.json['collateral_amount'], request.json['user_id'], request.json['dwelling_id'],
                        request.json['active'])
...
def register_deal():
    ...
    deal_date_now = datetime.datetime.now()
    user = RestUser.query.get(1)
    # if check_logged_user_in():
    #     user_id = check_user_id()
    dwelling = RestDwelling.query.get(1)
    user_id = user.id
    dwelling_id = dwelling.id
    active = True
    print(user_id)
    new_deal = RestDeal(deal_date_now, request.json['price'], request.json['rent_time'],
                        request.json['collateral_amount'], user_id, dwelling_id, active)
    db.session.add(new_deal)
    db.session.commit()
...
def update_deal(id):
    ...
    deal.deal_date = request.json['deal_date']
    ...
    deal.dwelling_id = request.json['dwelling_id']
